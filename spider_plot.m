...
elseif iscell(axes_shaded_limits)
    ...
    for ii = 1:length(axes_shaded_limits)
        ...
        if any(axes_shaded_limit(1, :) < axes_limits(1, :)) ||...
            ...
            for i=1:length(axes_shaded_limit(1, :))
                if axes_shaded_limit(1, i) < axes_limits(1, i)
                    axes_shaded_limit(1, i) = axes_limits(1, i);
                end
            end
            for i=1:length(axes_shaded_limit(2, :))
                if axes_shaded_limit(2, i) > axes_limits(2, i)
                    axes_shaded_limit(2, i) = axes_limits(2, i);
                end
            end
        end
    end
